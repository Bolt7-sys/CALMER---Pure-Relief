...
export default function ClientLayout() {
  ...
  return (
    <div className="min-h-screen bg-dark-radial max-w-md mx-auto relative pb-24">
      {/* Top header */}
      <header className="sticky top-0 z-30 glass-strong px-4 py-3 flex items-center justify-between">
        <Logo size={38} />
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/shop')} className="w-10 h-10 rounded-full glass grid place-items-center text-soft-gold">
            <i className="fa-solid fa-magnifying-glass"></i>
          </button>
          <button onClick={() => { navigate('/notifications'); setUnread(0) }}
      ...
      {/* Bottom nav */}
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-40 glass-strong border-t border-rich-gold/10 px-2 py-2 flex justify-around">
        {NAV.map(n => (
          <NavLink key={n.to} to={n.to} end={n.end}
            ...
            <div className="relative">
              ...
              {n.to === '/cart' && count > 0 && (
                <span className="absolute -top-2 -right-3 min-w-[16px] h-4 px-1 rounded-full bg-gold-gradient text-black text-[9px] font-bold grid place-items-center">{count}</span>
              )}
            </div>
